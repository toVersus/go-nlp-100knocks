...
var insertDocsTests = []struct {
	...
	{
		name: "should retrieve multiple documents by alias",
...
{"name": "Danièle Forget", "sort_name": "Forget, Danièle", "ended": true, "gid": "5d6d5857-3d03-4b1b-a3ee-d789e883c1b2", "type": "Person", "id": 726135}`,
		alias: "Sam James Vende",
		want:  []string{"Sam James"},
	},
}
...
func TestGetTags(t *testing.T) {
	for _, testcase := range insertDocsTests {
		...
		var results Artists
		if err := c.Find(bson.M{"aliases.name": testcase.alias}).All(&results); err != nil {
			t.Error(err)
		}

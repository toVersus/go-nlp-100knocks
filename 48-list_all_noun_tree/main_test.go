...
EOS`,
	...
	{
		name: "should return noun tree of phrases",
		text: `EOS
* 0 26D 0/1 -1.536722
吾輩	名詞,代名詞,一般,*,*,*,吾輩,ワガハイ,ワガハイ
は	助詞,係助詞,*,*,*,*,は,ハ,ワ
...
`,
		expect: `吾輩は -> 定めた
彼と -> 近付に -> なってから -> 飲み込んだから -> わるくするのも -> 愚である -> 若くはないと -> 定めた
近付に -> なってから -> 飲み込んだから -> わるくするのも -> 愚である -> 若くはないと -> 定めた
呼吸を -> 飲み込んだから -> わるくするのも -> 愚である -> 若くはないと -> 定めた
場合にも -> わるくするのも -> 愚である -> 若くはないと -> 定めた
己れを -> 弁護して -> わるくするのも -> 愚である -> 若くはないと -> 定めた
弁護して -> わるくするのも -> 愚である -> 若くはないと -> 定めた
形勢を -> わるくするのも -> 愚である -> 若くはないと -> 定めた
わるくするのも -> 愚である -> 若くはないと -> 定めた
愚である -> 若くはないと -> 定めた
事彼に -> しゃべらして -> 濁すに -> 若くはないと -> 定めた
自分の -> 手柄話を -> しゃべらして -> 濁すに -> 若くはないと -> 定めた
手柄話を -> しゃべらして -> 濁すに -> 若くはないと -> 定めた
御茶を -> 濁すに -> 若くはないと -> 定めた
思案を -> 定めた
`,
	},
...
func TestNounTree(t *testing.T) {
	for _, testcase := range nounTreeTests {
		t.Log(testcase.name)

		r := strings.NewReader(testcase.text)
		result := newChunkPassage(r).stringifyNounTree()
...
func BenchmarkNounTree(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		for _, testcase := range nounTreeTests {
			r := strings.NewReader(testcase.text)
			s = newChunkPassage(r).stringifyNounTree()
		}
		result = s
	}
}

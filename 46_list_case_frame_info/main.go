...
func main() {
	...
	content := newChunkPassage(f).stringifyCaseFrame()
...
// stringifyCaseFrame formatted case frame information.
func (chunkPassage ChunkPassage) stringifyCaseFrame() string {
	var buf bytes.Buffer
	for _, passage := range chunkPassage {
		for i, chunk := range passage {
			var verb, phrases string
			if chunk.srcs == nil {
				continue
			}
			...
			phrases = getCaseFrame(i, passage)
			if phrases == "" {
				continue
			}
			buf.WriteString(verb + "\t" + phrases)
		}
	}
...
func getCaseFrame(currentIndex int, passage []Chunk) string {
	var particles, phrases []string
	var buf bytes.Buffer
	for _, src := range passage[currentIndex].srcs {
		...
		phrases = append(phrases, dict)
	}

	if len(particles) == 0 || len(phrases) == 0 {
		return ""
	}

	buf.WriteString(strings.Join(particles, " ") + "\t" + strings.Join(phrases, " ") + "\n")
	return buf.String()
}

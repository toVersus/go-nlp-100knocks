...
func main() {
	...
	morphs.sortByAppearance().drawBarCharts("top10-commonly-used-words.png")
}
...
// drawBarCharts draws bar charts
func (counters sortCounters) drawBarCharts(filename string) error {
	group := plotter.Values{}
	var labels []string

	p, err := plot.New()
	if err != nil {
		return fmt.Errorf("could not create a new plot: %s", err)
	}

	for i := 0; i < 10; i++ {
		group = append(group, float64(counters[i].Count))
		labels = append(labels, counters[i].Key)
	}

	p.Title.Text = "Top 10 commonly used words"
	...
	if err != nil {
		return fmt.Errorf("could not create a new bar chart: %s", err)
	}
	...
	if err := p.Save(5*vg.Inch, 3*vg.Inch, filename); err != nil {
		return fmt.Errorf("could not save a bar chart: %s", err)
	}

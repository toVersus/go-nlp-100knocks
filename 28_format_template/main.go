...
var redirectTagReg = regexp.MustCompile(`#REDIRECT \[\[.*\]\]`)
var commentoutTagReg = regexp.MustCompile(`\<\!\-\-.*\-\-\>`)
var referenceTagReg = regexp.MustCompile(`\{\{.*\}\}`)
...
func (articles Articles) getTemplate() map[string]string {
	...
	for _, a := range articles {
		for _, line := range templateFieldReg.FindAllStringSubmatch(a.Text, -1) {
			...
			// Removing comment-out symbols.
			// https://en.wikipedia.org/wiki/Help:Cheatsheet
			if commentoutTagReg.MatchString(line[2]) {
			...
			// Removing redirect symbols to a section of another page.
			// https://en.wikipedia.org/wiki/Help:Cheatsheet
			if redirectTagReg.MatchString(line[2]) {
				line[2] = strings.Replace(line[2], "#REDIRECT ", "", -1)
				line[2] = strings.Replace(line[2], "[[", "", -1)
				line[2] = strings.Replace(line[2], "]]", "", -1)
			}

			// Removing internal link (link to a section) synbols.
			// https://en.wikipedia.org/wiki/Help:Cheatsheet
			if internalLinkReg.MatchString(line[2]) {
				line[2] = strings.Replace(line[2], "[[", "", -1)
				line[2] = strings.Replace(line[2], "]]", "", -1)
